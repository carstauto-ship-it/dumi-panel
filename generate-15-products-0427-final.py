#!/usr/bin/env python3
"""Regenerate 15 acoustic panel product pages + update products.html + sitemap.xml"""

import os

PRODUCTS_DIR = '/Users/carstauto/.openclaw/workspace/dumi-panel/products'
PRODUCTS_HTML = '/Users/carstauto/.openclaw/workspace/dumi-panel/products.html'
SITEMAP_XML = '/Users/carstauto/.openclaw/workspace/dumi-panel/sitemap.xml'
...
products = [
    ...
    },
    {
        ...
        "description": "Ceiling Hanging Acoustic Baffles are vertically suspended acoustic panels designed to absorb sound in spaces with high ceilings where wall and ceiling surface area is insufficient for effective acoustic treatment. These baffles are commonly used in gymnasiums, swimming pools, warehouses, and transit stations where reverberation control is challenging due to large volumes of air.",
        "price": "From $45/m²",
        "image": "../images/akupanelcn/ceiling-1.jpg",
        "features": [
    ...
    },
    {
        ...
        "price": "From $95/m²",
        "image": "../images/akupanelcn/3d-panel-1.jpg",
        "features": [
...
# Image mapping for products.html cards
IMAGE_MAP = {
    "acoustic-bass-traps": ("../images/akupanelcn/bass-trap-1.jpg", "Acoustic Bass Traps"),
    "fabric-wrapped-acoustic-panels": ("../images/akupanelcn/fabric-1.jpg", "Fabric Wrapped Acoustic Panels"),
    "perforated-wood-acoustic-panels": ("../images/akupanelcn/wood-slat-1.jpg", "Perforated Wood Acoustic Panels"),
    "melamine-foam-acoustic": ("../images/akupanelcn/concertina-foam-1.jpg", "Melamine Foam Acoustic Panels"),
    "glass-wool-acoustic-panels": ("../images/akupanelcn/ceiling-1.jpg", "Glass Wool Acoustic Panels"),
    "ceiling-hanging-baffles": ("../images/akupanelcn/ceiling-1.jpg", "Ceiling Hanging Acoustic Baffles"),
    "perforated-metal-ceiling": ("../images/akupanelcn/perforated-metal-1.jpg", "Perforated Metal Ceiling Panels"),
    "resilient-bar-channel": ("../images/akupanelcn/acoustic-door-1.jpg", "Resilient Bar Channel Systems"),
    "acoustic-door-panels": ("../images/akupanelcn/acoustic-door-1.jpg", "Acoustic Door Panels"),
    "mass-loaded-vinyl-sheets": ("../images/akupanelcn/acoustic-door-1.jpg", "Mass Loaded Vinyl Sheets"),
    "window-acoustic-glazing": ("../images/akupanelcn/acoustic-window-1.jpg", "Window Acoustic Glazing Units"),
    "stadium-acoustic-panels": ("../images/akupanelcn/echo-absorber-1.jpg", "Stadium Acoustic Panels"),
    "transportation-acoustic-panels": ("../images/akupanelcn/micropore-1.jpg", "Transportation Acoustic Panels"),
    "marine-acoustic-panels": ("../images/akupanelcn/diffuser-1.jpg", "Marine Acoustic Panels"),
    "custom-shaped-acoustic-panels": ("../images/akupanelcn/3d-panel-1.jpg", "Custom Shaped Acoustic Panels"),
}
...
{json_ld}
    ...
    <meta property="og:image" content="https://dumi-panel.com/{image}">
...
def add_to_products_html():
    """Add missing products to products.html"""
    with open(PRODUCTS_HTML, 'r') as f:
        content = f.read()
    
    # Find insertion point: before </div> that closes the products grid
    # The pattern before the last product card ends with </a>
    # We need to insert before the closing </div> of the product-grid
    
    new_entries = []
    for slug, (img_path, display_name) in IMAGE_MAP.items():
        # Convert ../images/ to images/ for the products.html path
        img_src = img_path.replace('../', '')
        entry = f'''                            <a href="products/{slug}.html" class="product-card">
                    <div class="product-img"><img src="{img_src}" alt="{display_name}" style="width:100%;height:100%;object-fit:cover;"></div>
                    <div class="product-info"><h3>{display_name}</h3><p>DUMI Professional Series</p><span class="product-price">View Details →</span></div>
                </a>
'''
        new_entries.append(entry)
    
    # Insert before the last </div> that closes the product grid
    # Look for the pattern: the last </a> followed by newline and </div>
    # We'll insert just before the </div> that closes the grid
    
    # Find the insertion point: after the last product card, before the closing </div>
    # The products.html ends with: ...product-card...</a>\n            </div>\n    </section>
    # So we insert after </a>\n            and before </div>
    
    # Actually, let me look for a safer anchor: the </div> that closes product-grid
    # We'll use the pattern where we find the last occurrence of a product card and insert after it
    
    # More reliable: find the last </a> followed by newline and whitespace and </div>
    import re
    
    # Find the pattern where product cards end
    # Insert new entries just before the closing </div> of the grid
    pattern = r'(\n\s+</div>\n\s+</section>\n<footer>)'
    if re.search(pattern, content):
        # Insert before this closing </div>
        new_entries_str = '\n' + '\n'.join(new_entries) + '\n'
        content = re.sub(pattern, '\n' + new_entries_str + r'\1', content)
    else:
        # Fallback: find the last product card and insert after
        print("WARNING: Could not find exact insertion point, using fallback")
        # Just append before </div> of the container
        new_entries_str = '\n' + '\n'.join(new_entries) + '\n'
        content = content.replace('\n            </div>\n    </section>', '\n' + new_entries_str + '            </div>\n    </section>')
    
    with open(PRODUCTS_HTML, 'w') as f:
        f.write(content)
    
    print(f"Added {len(new_entries)} products to products.html")


def update_sitemap():
    """Add missing products to sitemap.xml"""
    ...
    for product in products:
        slug = product['slug']
        url = f"https://dumi-panel.com/products/{slug}.html"
...
def main():
    ...
    for product in products:
        ...
        print(f"Created/Updated: {product['filename']}")

    print(f"\nTotal product pages created/updated: {len(created)}")
    
    add_to_products_html()
    update_sitemap()
